...
"""
usage: get_mc_DMRs.py [-h] -x HOST -u USER -p PASSWORD -d DATABASE -f FILE
...
class mcDatabase:
    ...
    def gatherDataFromMySQL(self, line, context):
        ...
        for i in self.tables:
            query = "select sum(mc), sum(h) from {db} where assembly = {ch} and position between {s} and {e}".format(
                db=str(i),
                ch=line[0].strip('chr'),
                s=line[1],
                e=line[2])
